...
def run_self_check(report_path=None):
    ...
    required_modules = [
        "bactassocnet.config",
        "bactassocnet.pipeline",
        "bactassocnet.io.loader",
    ]
    ...
    anti_salami = {
        "novelty_axis": "Marginal+conditional association engine (MI/phi/CMH/CMI) + direct-network (Ising) + topology summaries",
        "scope_boundary": "No MDR phenotype ontology/classification; no phylogenetic trait-evolution model",
        "methodological_uniqueness": "Association effect sizes (OR/CI, phi, MI), graph topology, confounder-stratified analyses",
        "primary_user_story": "Association network inference for binary features with confounder-aware options",
