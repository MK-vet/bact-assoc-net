...
def run_self_check(report_path=None):
    ...
    checks=[]
    required_modules=['bactassocnet.config','bactassocnet.pipeline','bactassocnet.io.loader']
    optional_modules=['bactassocnet.gui.launch']
    ...
    anti_salami = {"novelty_axis": "Marginal+conditional association engine (MI/phi/CMH/CMI) + direct-network (Ising) + topology summaries", "scope_boundary": "No MDR phenotype ontology/classification; no phylogenetic trait-evolution model", "methodological_uniqueness": "Association effect sizes (OR/CI, phi, MI), graph topology, confounder-stratified analyses", "primary_user_story": "Association network inference for binary features with confounder-aware options", "recommended_softwarex_validation": ["runtime benchmark scaling (synthetic n,p)", "smoke test on real biological CSV/Newick with saved outputs", "reproducibility via run_manifest + config snapshot", "documented limitations and domain assumptions"]}
    out={
      'schema_version':'1.0',
      'report_type':'validation_self_check',
      'tool':'bact-assoc-net',
      'package':'bactassocnet',
      'version':__version__,
