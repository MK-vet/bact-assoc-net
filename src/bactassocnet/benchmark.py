...
def run_benchmark_paper_ready(
    ...
    if not df.empty:
        ...
        plt.close(fig)
    try:
        run_benchmark_compare_models(
            out_dir, n_rows=n_rows, p_values=p_values, seed=seed
        )
    except Exception:
        pass
    return js
...
def _edge_set_from_df(df, top_k=100):
    ...
    if "IsingDirect" in d.columns:
        ...
        return set()
    try:
        d = d.assign(_abs=d[col].astype(float).abs()).sort_values(
    ...
    d = d.head(int(top_k))
    return {
        tuple(sorted((str(a), str(b)))) for a, b in zip(d["Feature_A"], d["Feature_B"])
    }
...
def run_benchmark_compare_models(
    out_dir, n_rows=180, p_values=(1000, 5000, 20000), seed=123, boot_reps=1
):
    from pathlib import Path
    import json
    import time
    ...
    for p in p_values:
        ...
        methods = []
        t = time.perf_counter()
        plm = _plm_ising(Xsub, alpha=0.02)
        methods.append(("PLM-L1", plm, time.perf_counter() - t))
        try:
            t = time.perf_counter()
            gdf = _glasso_edges_from_binary(Xsub, alpha=0.02)
            tg = time.perf_counter() - t
        except Exception:
            gdf = pd.DataFrame(columns=["Feature_A", "Feature_B", "IsingDirect"])
            tg = float("nan")
                ...
                ers.append(
                    {
                        "Feature_A": feats[i],
                        "Feature_B": feats[j],
                        "phi": float(corr[i, j]) if np.isfinite(corr[i, j]) else 0.0,
                    }
                )
        t = time.perf_counter()
        ldf = glasso_ising_proxy(
            pd.DataFrame(ers), feature_matrix=None, thr=0.0, alpha=0.02
        )
        methods.append(("LegacyPrecisionOnPhi", ldf, time.perf_counter() - t))
        ...
        for mname, d0, rt in methods:
                                ...
                                er.append(
                                    {
                                        ...
                                        "Feature_B": fb[j],
                                        "phi": float(c2[i, j])
                                        if np.isfinite(c2[i, j])
                                        else 0.0,
                                    }
                                )
                        db = glasso_ising_proxy(
                            pd.DataFrame(er), feature_matrix=None, thr=0.0, alpha=0.02
                        )
                    a = refs.get(mname, set())
                    b = _edge_set_from_df(db)
            ...
            rows.append(
                {
                    "tool": "bact-assoc-net",
                    "benchmark": "ising_compare",
                    "p": int(p),
                    "p_used": int(feat_cap),
                    "method": mname,
                    "runtime_sec": None if rt != rt else round(float(rt), 4),
                    "n_edges": int(len(d0)) if d0 is not None else 0,
                    "sparsity_edges_per_feature": round(
                        (len(d0) / max(1, feat_cap)) if d0 is not None else 0.0, 4
                    ),
                    "stability_jaccard_mean": round(float(np.nanmean(jaccs)), 4)
                    if np.isfinite(np.nanmean(jaccs))
                    else None,
                    "stability_jaccard_sd": round(float(np.nanstd(jaccs)), 4)
                    if np.isfinite(np.nanstd(jaccs))
                    else None,
                }
            )
    out = {
        "tool": "bact-assoc-net",
        "report": "ising_model_comparison",
        "results": rows,
    }
    (out_dir / "benchmark_ising_model_comparison.json").write_text(
    ...
    if not df.empty:
        ...
        try:
            ...
            for y, fn in [
                ("runtime_sec", "benchmark_ising_runtime_comparison.png"),
                (
                    "sparsity_edges_per_feature",
                    "benchmark_ising_sparsity_comparison.png",
                ),
                ("stability_jaccard_mean", "benchmark_ising_stability_comparison.png"),
            ]:
                fig = plt.figure(figsize=(6, 4))
                for m, sub in df.groupby("method"):
                    plt.plot(sub["p"], sub[y], marker="o", label=m)
                plt.xscale("log")
                plt.xlabel("p (synthetic features)")
                plt.ylabel(y)
                plt.legend()
                plt.tight_layout()
