...
from .information_theory.core import (
    chi2_phi,
    mutual_information,
    adaptive_phi_threshold,
)
from .information_theory.advanced import surrogate_mi
from .innovation_v3 import (
    glasso_ising_proxy,
    topology_persistence_from_sweep,
    multilayer_dispersion,
)
...
class Pipeline:
    ...
    def _write_manifest(
        self, used_layers: List[LayerSpec], extra_inputs: List[str]
    ) -> None:
        import platform
        import sys
    ...
    def run(self) -> Dict[str, pd.DataFrame]:
        ...
        all_ids = (
            sorted(set.intersection(*id_sets))
            if cfg.input.align_mode.lower() == "intersection"
            else sorted(set.union(*id_sets))
        )
        if not all_ids:
        ...
        for sp in cfg.layers:
            ...
            if cfg.input.drop_samples_with_missing:
                ...
                df_qc = df_qc.loc[obs]
            df_qc = df_qc.loc[
                :, (df_qc.isna().mean(axis=0) <= cfg.input.max_missing_feature)
            ]
        ...
        for lname, df in layers.items():
            ...
            merged = merged.join(
                df.rename(columns=dict(zip(df.columns, cols))), how="left"
            )
        ...
        extra_inp = []
        if (
            cfg.conditional.enabled
            and cfg.conditional.confounder_files
            and cfg.conditional.confounders
        ):
            parts = []
        ...
        for i, j in combinations(range(len(cols)), 2):
            ...
            mi = float(
                mutual_information(
                    x[mxy].astype(int).to_numpy(), y[mxy].astype(int).to_numpy()
                )
            )
        ...
        if net.phi_method.lower() == "adaptive":
            thr_phi = adaptive_phi_threshold(
                edges_df["phi"].to_numpy(),
                method="percentile",
                percentile=int(net.phi_percentile),
            )
        else:
            thr_phi = float(net.phi_fixed)
        alpha = float(net.alpha)
        keep = (
            (edges_df["N"] >= net.min_pairwise_n)
            & (edges_df["p"] <= alpha)
            & (edges_df["phi"].abs() >= thr_phi)
        )
        ...
        summ = [
            {
                ...
                "AvgDegree": float(np.mean([d for _, d in G.degree()]))
                if G.number_of_nodes()
                else 0.0,
            }
        ]
        ...
        sweep = _simplicial_sweep(
            G, thresholds=[0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4]
        )
        ...
        for la, lb in combinations(layer_cols.keys(), 2):
            m = edges_df[
                (
                    edges_df["Feature_A"].str.startswith(la + "::")
                    & edges_df["Feature_B"].str.startswith(lb + "::")
                )
                | (
                    edges_df["Feature_A"].str.startswith(lb + "::")
                    & edges_df["Feature_B"].str.startswith(la + "::")
                )
            ]
            if m.empty:
                continue
            rows.append(
                {
                    ...
                    "Layer_B": lb,
                    "N_pairs": int(len(m)),
                    "MI_mean": float(m["MI"].mean()),
                    "MI_median": float(m["MI"].median()),
        ...
        try:
            direct = glasso_ising_proxy(
                E if not E.empty else edges_df,
