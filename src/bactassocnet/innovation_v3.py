...
def _wide_from_edges(edges_df: pd.DataFrame):
    ...
    wcol = (
        "phi"
        if "phi" in edges_df.columns
        else ("MI" if "MI" in edges_df.columns else None)
    )
    if wcol is None:
        return feats, S
...
def glasso_ising_proxy(
    ...
    if feature_matrix is not None:
        ...
        keep = [
            c
            for c in X.columns
            if X[c].notna().sum() >= 10 and X[c].dropna().nunique() > 1
        ]
        X = X[keep]
        if X.shape[1] >= 2:
            ...
            if not plm.empty:
                ...
                return plm.sort_values(
                    "IsingDirect", key=lambda s: s.abs(), ascending=False
...
def multilayer_dispersion(edges_df: pd.DataFrame) -> pd.DataFrame:
    ...
    all_layers = sorted(
        set(
            split_layer(x)
            for x in pd.concat([edges_df["Feature_A"], edges_df["Feature_B"]], axis=0)
        )
    )
    for f in sorted(set(edges_df["Feature_A"]).union(edges_df["Feature_B"])):
        ...
        if p.sum() == 0:
            continue
        p /= p.sum()
        m = np.ones_like(p) / len(p)
        jsd = 0.5 * np.sum(
            p * np.log2((p + 1e-12) / (0.5 * (p + m) + 1e-12))
        ) + 0.5 * np.sum(m * np.log2((m + 1e-12) / (0.5 * (p + m) + 1e-12)))
        part = 1.0 - np.sum(p**2)
        rows.append(
            {
                "Feature": f,
                "JSD_to_uniform_layers": float(jsd),
                "ParticipationCoeff": float(part),
                "Degree": int(len(sub)),
            }
        )
    return (
        pd.DataFrame(rows).sort_values(
            ["ParticipationCoeff", "Degree"], ascending=[False, False]
        )
        if rows
        else pd.DataFrame()
    )
