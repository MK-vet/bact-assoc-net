...
class TestSignalRecovery:
    ...
    def test_surrogate_mi_nonsignificant_for_independent(self, independent_pair):
        ...
        assert r["Z"] < 2.0, (
            f"False positive: Z={r['Z']:.2f} for independent pair"
        )
    ...
    def test_pid_gene0_unique_exceeds_gene1(self, planted_df):
        ...
        r = pid(planted_df["pheno_0"].values,
    ...
    def test_euler_characteristic(self):
        ...
        edf = pd.DataFrame({"Feature1": ["A","A","B"],
                             "Feature2": ["B","C","C"],
                             "phi": [0.8, 0.8, 0.8]})
...
class TestCLI:
    def test_help_exits_zero(self):
        r = subprocess.run([sys.executable, "-m", "bactassocnet.cli", "--help"],
                           capture_output=True)
        assert r.returncode == 0

    def test_version_output(self):
        from bactassocnet import __version__
        r = subprocess.run([sys.executable, "-m", "bactassocnet.cli", "--version"],
                           capture_output=True, text=True)
        assert __version__ in r.stdout

    def test_self_check_passes(self):
        r = subprocess.run([sys.executable, "-m", "bactassocnet.cli", "--self-check"],
                           capture_output=True, text=True)
        assert r.returncode == 0
    ...
    def test_run_produces_mandatory_outputs(self, planted_df, tmp_path):
        ...
        config = {
            ...
            "association": {"method": "phi", "n_surrogates": 50,
                            "threshold_method": "percentile", "threshold_value": 75},
            "n_jobs": 1, "seed": 0,
        }
        ...
        r = subprocess.run(
            [sys.executable, "-m", "bactassocnet.cli",
