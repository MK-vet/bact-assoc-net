...
def binary_pair():
    ...
    y = x.copy(); flip = rng.choice(200, 30, replace=False); y[flip] = 1 - y[flip]
    return x, y
...
def binary_df():
    ...
    df.loc[df["gene_0"] == 1, "pheno_0"] = rng.binomial(1, 0.9, (df["gene_0"] == 1).sum())
    return df
...
class TestSurrogateMI:
    def test_smi_positive_for_correlated(self, binary_pair):
        ...
        r = surrogate_mi(pd.Series(binary_pair[0]), pd.Series(binary_pair[1]), n_surrogates=50, seed=42)
        assert r["MI"] >= 0
...
class TestPID:
    ...
    def test_pid_dominant_field(self, binary_df):
        from bactassocnet.information_theory.advanced import pid
        r = pid(binary_df["pheno_0"].values,
                binary_df["gene_0"].values, binary_df["gene_1"].values)
...
class TestConfig:
    def test_resolve_layers_from_dir(self, tmp_path):
        ...
        # Create dummy CSVs
        for name in ["AMR", "VIR"]:
            pd.DataFrame({"Strain_ID": ["S1"], "f": [1]}).to_csv(tmp_path / f"{name}.csv", index=False)
